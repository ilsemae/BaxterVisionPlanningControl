...
def define_major_points():
    ...
    #time.sleep(8)
    input("Place arm at our own goal center. ")
    ...
    #time.sleep(8)
    input("Place arm above origin corner of the board:")
    pose=limb.endpoint_pose()
    opos=pose['position']

    #print("Place the arm at the tip of your x unit vector.")
    #time.sleep(8)
    input("Place the ball at the tip of your x unit vector.")
    pose=limb.endpoint_pose()
    xpos=pose['position']

    #print("Place the arm at the tip of your y unit vector.")
    #time.sleep(8)
    input("Place the ball at the tip of your y unit vector.")
    pose=limb.endpoint_pose()
    ypos=pose['position']
    ...
    input("Put ball on the negative y axis of the board.")
    yaxis = images[1]
    time.sleep(3)
    
    input("Put ball on the x axis of the board.")
    xaxis = images[1]
    time.sleep(3)
...
#    rospy.set_param('max_operating_height',limb_endxyz.z)
    ...
    (yaxis_pix_y,yaxis_pix_x)=findball_bb(img2cimg(yaxis,lower,upper),yaxis)
    (xaxis_pix_y,xaxis_pix_x)=findball_bb(img2cimg(xaxis,lower,upper),xaxis)
...
def getBoardState():
    ...
    for i in range(0,len(color_boundaries)):
        ...
        if np.array_equal(lower,color_boundaries[0][0]) and np.array_equal(upper,color_boundaries[0][1]):
            #print("pink")
            ball_x, ball_y, ball_on_ground = find_ball(cimg)
        ...
        if np.array_equal(lower,color_boundaries[2][0]) and np.array_equal(upper,color_boundaries[2][1]):
            #print("blue")
            blocks_x, blocks_y, blocks_theta = find_blocks(cimg)
    ...
    if abs(ball_x-board_home[0]) < .001 and abs(ball_y-board_home[1]) < 001:
        defense = False
    else:
        defense = True
...
if __name__ == "__main__":
    ...
    rospy.set_param('hand_mode','right')
    print("eyes seeing")
